...
CREATE TABLE standard_user (
	user_id integer PRIMARY KEY AUTO_INCREMENT, /* autoincremented id */
...
CREATE TABLE moderator (
	moderator_id integer PRIMARY KEY AUTO_INCREMENT, /* autoincremented id */
...
CREATE TABLE email (
	email varchar (40) NOT NULL, /*mail address. Constraints check the string against a regex matching: one or more alphanumeric characters or special symbols, followed by an “at”, followed by one or more alphanumeric character, followed by a point and one or more alphanumeric characters. The regex expresses the usual structure of a mail, in the form “username@domainname.topleveldomain*/ 
...
CREATE TABLE ad (
	ad_id integer PRIMARY KEY AUTO_INCREMENT, /* autoincremented id */
	...
	status integer NOT NULL, /* 1 = approved, 2 = rejected, 3 = pending, 4 = out of date */
...
CREATE TABLE image (
	...
	image_only integer NOT NULL, /* integer acting as boolean variable: 0 = support image, such images that describe ad together with text; 1 = only image, such that the text description is not present and is substituted by the image */
